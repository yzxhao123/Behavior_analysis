...
def detect_img(img,model):
    ...
    return a_frame,  result_text


##################tracking##############################################
def track_img(img, model):
    ...
    # 提取边界框和跟踪 ID
    boxes = results[0].boxes.xywh.cpu()  # 中心点坐标 (x, y) 和宽高 (w, h)
    track_ids = results[0].boxes.id.int().cpu().tolist()  # 跟踪 ID
